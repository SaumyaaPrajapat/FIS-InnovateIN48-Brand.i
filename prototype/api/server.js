...
// Endpoint to calculate compliance score
app.get('/calculate-compliance-score', (req, res) => {
    try {
        const filePath = './compliance_report/compliance_score.json';
        if (fs.existsSync(filePath)) {
            const jsonData = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            console.log(jsonData);
            const { complianceScore } = jsonData;
            res.json({ complianceScore });
        }
        else
            res.json({ 'message': 'Compliance report has not been generated yet, so no score is provided.' }); // Send the score as a JSON response
    } catch (error) {
        console.error('Error calculating compliance score:', error);
        res.status(500).json({ error: 'Error calculating compliance score' });
    }
});
